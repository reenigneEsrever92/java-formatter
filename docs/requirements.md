...
| R35 | The remaining import-on-demand options are honoured: `NAMES_COUNT_TO_USE_IMPORT_ON_DEMAND` (default 3) collapses one owner's static member imports into `import static pkg.Owner.*;` above the count, `PACKAGES_TO_USE_IMPORT_ON_DEMAND` (default `java.awt`, `javax.swing`; serialized as the nested `<value><list>` of `.*`-suffixed `<option>` entries, stripped on parse) always collapses a listed package's single-type imports into `pkg.*` regardless of count, and `USE_SINGLE_CLASS_IMPORTS` (default true), off, collapses every eligible non-static group into `pkg.*` — each merged group emitted at its first import's position, all keeping the conservative wildcard-present / same-name-ambiguity / local-type guards of the shipped class-count merge, the change is whitespace/layout only (R5, R6), absent and default options keep today's class-count-only merging byte-for-byte on the existing fixtures except that the built-in package list now folds lone `java.awt` / `javax.swing` single imports. | U1, UC1/UC2 | functional | medium (delivered 2026-09-05) |
| R36 | Javadoc layout follows the scheme's javadoc options: `ENABLE_JAVADOC_FORMATTING` gates the engine (IntelliJ's built-in default `true` ships as `false` — a recorded divergence keeping absent / default schemes byte-identical), and with the gate on a standalone `/** … */` comment whose structure parses cleanly is laid out per the `JD_*` options — description line breaks kept per `JD_PRESERVE_LINE_FEEDS` or merged per paragraph, empty description lines kept per `JD_KEEP_EMPTY_LINES` and rendered as `<p>` per `JD_P_AT_EMPTY_LINES`, a blank line after the description per `JD_ADD_BLANK_AFTER_DESCRIPTION`, `@param` descriptions aligned per `JD_ALIGN_PARAM_COMMENTS` (own line per `JD_PARAM_DESCRIPTION_ON_NEW_LINE`, blank after the block per `JD_ADD_BLANK_AFTER_PARM_COMMENTS`), `@throws` / `@exception` aligned per `JD_ALIGN_EXCEPTION_COMMENTS` and normalised to `@throws` per `JD_USE_THROWS_NOT_EXCEPTION`, a blank after `@return` per `JD_ADD_BLANK_AFTER_RETURN`, empty tags dropped per `JD_KEEP_EMPTY_PARAMETER` / `JD_KEEP_EMPTY_EXCEPTION` / `JD_KEEP_EMPTY_RETURN`, unknown tags per `JD_KEEP_INVALID_TAGS`, continuation lines indented per `JD_INDENT_ON_CONTINUATION`, the leading `*` per `JD_LEADING_ASTERISKS_ARE_ENABLED`, and one-line javadoc kept or expanded per `JD_DO_NOT_WRAP_ONE_LINE_COMMENTS`; the rewrite is whitespace/layout only — prose, tags and inline `{@code …}` / `{@link …}` text preserved in order, anything unparseable echoed byte-for-byte (R4), and the output re-formats to itself (R6); `CLASS_NAMES_IN_JAVADOC` parses (int 1–3) but its type-reference rewriting is not applied (safely ignored, R7). | U1, UC1/UC2 | functional | medium (delivered 2026-09-05) |
| R37 | Enum constant-list layout follows the scheme's enum options: `ENUM_CONSTANTS_WRAP` (default do not wrap, code `0`) lays out a constant-only body — no `;` declarations section — whose constants each render on one line as the flat `{A, B}` form, always under code `0` (a list that overflows the margin stays on one line), one constant per line under code `2`, and flat iff the flat declaration fits the margin under codes `1` / `5` (identical at this granularity — constants are echoed verbatim, so nothing is chopped inside a constant), while an enum with a `;` / member-declarations section or a constant that cannot render on a single line (multi-line source constant, own-line `ENUM_FIELD_ANNOTATION_WRAP` placement) keeps the expanded one-constant-per-line layout; `SPACE_INSIDE_ONE_LINE_ENUM_BRACES` (default false) pads the flat body only (`enum E { A, B }`), never the expanded layout — whitespace/layout only (R5), every produced layout re-formats to itself (R6), and the only pre-existing golden change is the default-scheme collapse of a fitting constant-only enum in the space_before_class_lbrace fixtures (the new default, matching IntelliJ). | U1, UC1/UC2 | functional | low (delivered 2026-09-06) |
| R38 | Chained builder-method calls follow the scheme's builder options: `BUILDER_METHODS` (default `""`; the comma-separated XML value is split on parse — trimmed, empties dropped — and joined with `,` on serialize, round-tripping exactly) names the methods whose chains get builder treatment — a chain that wraps per `METHOD_CALL_CHAIN_WRAP` and whose every call matches the list breaks after the receiver so each `.call()` (including the first) starts its own line at the continuation indent, and `KEEP_BUILDER_METHODS_INDENTS` (default false) keeps those lines at the chain's own indentation instead of stepping a continuation indent — whitespace/layout only (R5), produced layouts re-format to themselves (R6), and absent / default options (empty list, false) keep the generic chain layout byte-for-byte on every pre-existing fixture. | U1, UC1/UC2 | functional | low (delivered 2026-09-06) |
| R39 | Generic type-argument and type-parameter spacing follows the four angle-bracket spacing options of the java.md "Type arguments and parameters" table: `SPACES_WITHIN_ANGLE_BRACKETS` (default false) pads inside the angle brackets of type arguments and type parameters (`< T >`, nested generics padded at every level, the empty diamond `<>` never padded), `SPACE_AFTER_CLOSING_ANGLE_BRACKET_IN_TYPE_ARGUMENT` (default false) inserts one space after the closing `>` of an explicit type-argument list where it abuts a following token (explicit method invocation `a.<T> b()`, `new` with explicit type args `new <T> Type()`, chained-call links `.<T> m()`; the nested `>`-in-`>` join of a plain type stays tight per the docs-table description), `SPACE_BEFORE_OPENING_ANGLE_BRACKET_IN_TYPE_PARAMETER` (default false) separates a class / interface / record name from its type-parameter list (`class Foo <T>`, composing with the shipped `SPACE_BEFORE_TYPE_PARAMETER_LIST` at the same join — either on inserts the single space, both default off keeps `class Foo<T>`), and `SPACE_AROUND_TYPE_BOUNDS_IN_TYPE_PARAMETERS` (default true) spaces the `&` between a type parameter's bounds (`T extends A & B`; off renders `T extends A&B` while the mandatory separating space around `extends` stays) — whitespace/layout only (R5), inserting/removing one space is idempotent (R6), and absent / default options keep R14's canonical generic output byte-for-byte. | U1, UC1/UC2 | functional | low (delivered 2026-09-06) |
| R40 | Text-block layout and multi-catch type-list wrapping follow the scheme's Text-blocks and Multi-catch options: `ALIGN_MULTILINE_TEXT_BLOCKS` (default false) realigns a multiline text block used as a statement value by shifting its content and closing-delimiter lines by one uniform delta so the first content line sits at the statement's canonical continuation column (relative indentation and the stripped string value preserved; a block whose content whitespace the shift would cut is echoed verbatim), `STRIP_WHITESPACE_FROM_BLANK_LINES_IN_TEXT_BLOCKS` (default false) trims whitespace-only blank lines inside a text block to empty wherever the literal is rendered — an intentional, opt-in, value-safe deviation limited to blank lines (visible content untouched) — and both default off so text blocks stay byte-identical verbatim echoes (R4) unless a scheme opts in; `MULTI_CATCH_TYPES_WRAP` (IntelliJ default `1` wrap as needed; `JavaStyle::default()` ships `DoNotWrap` — a recorded divergence keeping absent/default schemes byte-identical, like `RECORD_COMPONENTS_WRAP`) breaks an over-margin `catch (A \| B e)` type list one member per line with the `|` leading each continuation line at the first type's column when `ALIGN_TYPES_IN_MULTI_CATCH` is on (default true), else at the continuation indent (codes `1` / `5` share the per-member layout; a single-type catch never wraps; a one-line `try` collapse is abandoned when its type list must wrap) — whitespace/layout only (R5), produced layouts re-format to themselves (R6). | U1, UC1/UC2 | functional | low (delivered 2026-09-06) |
| R41 | Record-pattern `case` labels follow the scheme's Deconstruction patterns (Java 21) options: `DECONSTRUCTION_LIST_WRAP` (IntelliJ default `1` wrap as needed; `JavaStyle::default()` ships `DoNotWrap` — a recorded divergence keeping absent/default schemes byte-identical, like `RECORD_COMPONENTS_WRAP`) wraps a record pattern's component list in a `switch` label (codes `1` / `5` share the one-component-per-line layout, `0` keeps the single long line, `2` wraps even a list that fits) with the wrapped components padded under the first component per `ALIGN_MULTILINE_DECONSTRUCTION_LIST_COMPONENTS` (default true, else continuation indent), every component on its own line under `NEW_LINE_AFTER_LPAREN_IN_DECONSTRUCTION_PATTERN` (default true, else the first component stays after the `(`) with the `)` alone at the label indent under `RPAREN_ON_NEW_LINE_IN_DECONSTRUCTION_PATTERN` (default true, else it hugs the last component), `SPACE_WITHIN_DECONSTRUCTION_LIST` (default false) padding a paren that shares its line with a component, and `SPACE_BEFORE_DECONSTRUCTION_LIST` (default false) separating the type from its `(`; the single modelled shape is a `case` label carrying exactly one record pattern — type + component list, components from trimmed source text with nested patterns atomic — and every other label shape (type patterns, guards, comma-separated constants) stays verbatim (R4); the layout is wired into the colon-form groups, the arrow-form rules and the one-line switch collapse, whitespace/layout only (R5), produced layouts re-format to themselves (R6), and absent/default schemes keep today's single-line labels byte-for-byte. | U1, UC1/UC2 | functional | low (delivered 2026-09-06) |
...
The remaining import-on-demand merging options are delivered by R35
(`NAMES_COUNT_TO_USE_IMPORT_ON_DEMAND`, `PACKAGES_TO_USE_IMPORT_ON_DEMAND`
and `USE_SINGLE_CLASS_IMPORTS` — import-on-demand-extensions).

The enum constant-list layout options are delivered by R37
(`ENUM_CONSTANTS_WRAP` and `SPACE_INSIDE_ONE_LINE_ENUM_BRACES` — enum-layout).

The builder-method chain options are delivered by R38 (`BUILDER_METHODS` and
`KEEP_BUILDER_METHODS_INDENTS` — builder-method-layout).

The type-argument and type-parameter spacing options are delivered by R39
...
`SPACE_AROUND_TYPE_BOUNDS_IN_TYPE_PARAMETERS` — type-argument-spacing-options).

The record-pattern deconstruction layout options are delivered by R41
(`DECONSTRUCTION_LIST_WRAP`, `ALIGN_MULTILINE_DECONSTRUCTION_LIST_COMPONENTS`,
`NEW_LINE_AFTER_LPAREN_IN_DECONSTRUCTION_PATTERN`,
`RPAREN_ON_NEW_LINE_IN_DECONSTRUCTION_PATTERN`,
`SPACE_WITHIN_DECONSTRUCTION_LIST` and
`SPACE_BEFORE_DECONSTRUCTION_LIST` — deconstruction-pattern-layout).
