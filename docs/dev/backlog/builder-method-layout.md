...
- [ ] config.rs: add `builder_methods: Vec<String>` and
      ...
      default).
- [ ] config.rs: add `OptionValue::String(String)` and drop `Copy` from the
      ...
      round-trip).
- [ ] crates/gui: add the `OptionValue::String` arm to `option_row`
      ...
      editable).
- [ ] formatter.rs: add `is_builder_chain` and teach `fmt_chain` the builder
      ...
      unchanged).
- [ ] Add `tests/options/builder_methods.rs` + fixtures under
...
- [ ] Add `tests/options/keep_builder_methods_indents.rs` + fixtures under
...
- [ ] Wire both modules alphabetically into `tests/options.rs`; assert each new
      golden is idempotent by re-formatting the `*.out.java` with the same
      style (AC3, R6: whole suite green, goldens idempotent).
- [ ] If an IntelliJ installation is available, format the fixture chain with
...
- [ ] Docs + full suite: flip the two `docs/settings/common.md` "Builder method
