...
state: done
verified: { by: maintainer, at: 2026-09-04T20:57:38Z }
priority: medium
tags: [dev, formatter]
owner: maintainer
---
...
- [x] config.rs: add the six `bool` fields + `Default` values to `JavaStyle`;
...
- [x] formatter.rs: add the shared `comment` helper implementing the column-1
...
- [x] formatter.rs: route all six emit sites (program header L267-269,
...
- [x] tests: `line_comment_at_first_column.rs` + fixtures under
...
- [x] tests: `block_comment_at_first_column.rs` + fixtures (same shape for
      `/* */`) — AC1, AC2.
- [x] tests: `keep_first_column_comment.rs` + fixtures (first-column comment
      with `line_comment_at_first_column = false`; true keeps column 1,
      false indents; default keeps column 1) — AC1, AC2.
- [x] tests: `line_comment_add_space_on_reformat.rs` + fixtures
      (`//foo` → `// foo`; false unchanged; default false) — AC1, AC3.
- [x] tests: `line_comment_add_space_in_suppression.rs` + fixtures
      (`//noinspection X` gains a space while ordinary `//foo` is untouched;
      false unchanged; default false) — AC1, AC3.
- [x] tests: `wrap_comments.rs` + fixtures (long line and block comments at a
      tight `right_margin`; true wraps at the margin, false unchanged,
      default false) — AC1, AC3.
- [x] Register all six files in `tests/options.rs` via
      ...
      no-op) — AC1, AC4.
- [x] Docs: flip the six "General & comments" rows in docs/settings/common.md
      to ✅; add the six options to the README honoured-options table and a
      formatting-behaviour note; add the requirement row (R16 was already
      taken in the table — appended as R23) to docs/requirements.md; append
      the entry to docs/dev/changelog.md — AC5.
- [x] If an IntelliJ installation is available, format a representative
      ...
      space, wrap continuation); record the outcome in the changelog. No
      IntelliJ installation was available — the goldens follow the settings
      table in docs/settings/common.md and the request decisions, and the
      outcome is recorded in the changelog entry.
