...
- The keep-simple one-liners (`KEEP_SIMPLE_BLOCKS_IN_ONE_LINE`,
  ...
  _flush_ (`if (c) {use();}`) by default, matching IntelliJ's built-in
...
- The gap _before_ a parenthesis, brace or clause keyword follows the
...
It renders every option the formatter supports in ordered sections taken from
...
- Both panes are syntax-highlighted with tree-sitter and scroll in both
  ...
  guide in the preview marks the configured right margin (`RIGHT_MARGIN` /
  `SOFT_MARGINS`, hidden when it is `0`).
